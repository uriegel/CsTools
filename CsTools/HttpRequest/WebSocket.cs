...
public class WebSocket(string url, Func<WebSocket, string, Task> onMessage, Action onClosed)
{
    ...
    public async Task OpenAsync()
	{
		...
		await (tcpClient?.ConnectAsync(url.Host, url.Port) ?? Task.FromResult(0));
		...
		async void Check()
		{
			try
			{
				while (true)
				{
					...
					await (pongReceived?.Task ?? Task.FromResult(false));
	...
	public async Task SendJsonAsync(object jsonObject)
	{
		...
		try
		{
			...
			await (networkStream?.WriteAsync(bytes, 0, bytes.Length) ?? Task.FromResult(0));
		}
		finally
		{
			writeLocker.Release();
...
";
			...
			{
				var read = await (networkStream?.ReadAsync(buffer, pos, buffer.Length - pos) ?? Task.FromResult(0));
				var result = read == 0;
				if (!result)
				{
			...
			Func<WsDecodedStream?, Exception?, Task> action = async (wsDecodedStream, exception) =>
			{
				try
				{
					if (exception == null)
					{
						...
						if (synchronizationContext != null)
							synchronizationContext.Send(_ => onMessage(this, payload), null);
						else
							await onMessage(this, payload);
					}
					else
					{
						Close();
...
class WsDecodedStream : Stream
    ...
    readonly UrlExtracts url = new(url);
